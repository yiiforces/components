...
use caching\Cache;
use caching\CacheInterface;
use caching\TagDependency;
use exceptions\Exception;
use exceptions\DbException;
use exceptions\DbIntegrityException;
 ...
 */
abstract class Schema extends \base\BaseObject
{
     ...
     */
    public $exceptionMap = [
        'SQLSTATE[23' =>  DbIntegrityException::class,
    ];
    /**
    ...
    {
        $className = $this->columnSchemaClass;
    ...
    {
        TagDependency::invalidate($this->db->schemaCache, $this->getCacheTag());
        $this->_tableNames    = [];
        $this->_tableMetadata = [];
    }
    ...
    public function refreshTableSchema($name)
    {
        ...
        $this->db->schemaCache->delete($this->getCacheKey($rawName));
    }
     ...
     * @return Exception
     */
    public function convertException($e, $rawSql)
    {
        if ($e instanceof Exception)
            return $e;

        $exceptionClass = DbException::class;
    ...
    public function getServerVersion()
    {
        if ($this->_serverVersion === null) {
            $this->_serverVersion = $this->db->pdo->getAttribute(\PDO::ATTR_SERVER_VERSION);
        }
    ...
    {
        $cache = null;
        if (!in_array($name, $this->db->schemaCacheExclude, true))
    ...
    {
        if ($this->db->pdo->getAttribute(\PDO::ATTR_CASE) !== \PDO::CASE_UPPER) {
            return $row;
        }
